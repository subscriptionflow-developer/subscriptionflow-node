class  Repository {
    ...
    getRelatedList(id, related) {
      return this.client.apiRequest({
        method: 'get',
        path: `/api/v1/${this.module}/${id}/link/${related}`,
      });
    }
    ...
    getFilteredList(criteria) {
      return this.client.apiRequest({
        method: 'get',
        path: `/api/v1/${this.module}/search?${criteria}`,
      });
    }

    filter(criteria) {//Todo check format and test  
      return this.client.apiRequest({
        method: 'post',
        path: `/api/v1/${this.module}/filter?${criteria}`,
      });
    }
